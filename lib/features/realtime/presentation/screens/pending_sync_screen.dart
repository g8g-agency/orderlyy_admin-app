...
class _PendingSyncScreenState extends ConsumerState<PendingSyncScreen>
  ...
  Widget build(BuildContext context) {
    ...
    // Summary counts
    final inflightCount = activeQueue
        .where((op) => op.status == SyncOperationStatus.inflight)
        .length;
    final retryingCount = activeQueue
        .where((op) => op.status == SyncOperationStatus.retrying)
        .length;
    final failedCount = activeQueue
        .where(
          (op) =>
    ...
    final ordersOps = activeQueue
        .where((op) => op.type == SyncOperationType.saveOrder)
        .toList();
    final callsOps = activeQueue
        .where(
          (op) =>
  ...
  Widget _buildSummaryDivider(Color borderColor) {
    return Container(height: 32, width: 1, color: borderColor);
  }
                ...
                children: [
                  OutlinedButton(
                    onPressed: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Operation discarded. Server state kept.',
                          ),
                        ),
                      );
                    },
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      ref
                          .read(syncQueueProvider.notifier)
                          .markSuccess(op.operationId);
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Conflict resolved by overwriting server.',
                          ),
                        ),
                      );
                    },
