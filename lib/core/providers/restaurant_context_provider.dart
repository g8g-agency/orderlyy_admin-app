...
final restaurantContextRepositoryProvider = Provider<RestaurantContextRepository>((ref) {
...
class RestaurantContextNotifier extends StateNotifier<RestaurantContextState> {
  ...
  RestaurantContextNotifier(this._repository, this._ref) : super(const RestaurantContextState());
  ...
  Future<void> bootstrapContext({String? branchId}) async {
    ...
    if (result is Success<RestaurantContextDto>) {
      state = state.copyWith(isLoading: false, context: result.data);
      ...
      if (result.failure.code == ApiErrorCode.unauthorized || 
          result.failure.code == ApiErrorCode.forbidden) {
  ...
  bool hasFeature(String featureKey) {
    if (state.context == null) return false;
    return state.context!.featureAccess.any((f) => f.featureKey == featureKey && f.isEnabled);
  }
...
final restaurantContextProvider = StateNotifierProvider<RestaurantContextNotifier, RestaurantContextState>((ref) {
